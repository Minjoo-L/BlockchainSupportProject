...
async function query1(name, params) {
    ...
    var peer = fabric_client.newPeer('grpc://localhost:27051');
...
async function query3(func, params){
	var key, name, id, address, job, story, age, sex, status;
		key = params[3]
		name = params[0]
		age = params[1]
		sex = params[2]
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
			...
			var request;
			switch (func) {
				case 'recipientNonIdent':
					request = {
						//targets : --- letting this default to the peers assigned to the channel
						chaincodeId: 'test-app4',
						txId: tx_id,
						fcn: 'recipientNonIdent',
						args: [name, age, sex, id, address, job, story, status]
					};
					break;
				default:
					break;
...
async function query4(func, params){
